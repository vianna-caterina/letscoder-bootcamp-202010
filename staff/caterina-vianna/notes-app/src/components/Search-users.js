...
function SearchUser() {
  ...
  const handleOnChange = (event) => {
    ...
    if (query) handleSearchUser(query);
    else setResults([]);
    ...
    <>
      <form className="searchUser" onSubmit={(event) => event.preventDefault()}>
        ...
        {(results && results.length && (
          <ul className="searchUser__ul">
            {results.map(({ fullname, _id }) => (
              <li key={_id} className="searchUser__li">
                {fullname && (
                  <p className="searchUser__result-name">{fullname}</p>
                )}
